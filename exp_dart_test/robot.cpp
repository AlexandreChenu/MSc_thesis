...
namespace robot_dart {
    ...
    {   
        ...
        int n_Dof = _skeleton->getNumDofs();

        Eigen::VectorXd commands = Eigen::VectorXd::Zero(n_Dof);

        std::vector<float> inputs(2 + n_Dof + 3); //HOW MANY ENTRIES?

        //auto pos?
        auto pos=_skeleton->getPositions().head(6).tail(3).cast <float> (); //Obtain robot pos 
        inputs[0] = pos[0] - target[0];
        inputs[1] = pos[1] - target[1];

        Eigen::VectorXd prev_commands = _skeleton->getCommands(); //get previous command -> to test 
        ...
        //auto angles? 
        auto angles = _skeleton->getPositions().head(6).head(3).cast <float> (); //Obtain robot pos 

        double yaw = angles[0]; //how to access orientation
        double pitch = angles[1];
        double roll = angles[2];

        inputs[1 + n_Dof + 1] = yaw;
        inputs[1 + n_Dof + 2] = pitch;
        inputs[1 + n_Dof + 3] = roll;

        model.gen().init();
        for (int j = 0; j < model.gen().get_depth() + 1; ++j)
            model.gen().get_depth(inputs); 

        Eigen::VectorXd out_nn;
        for (int indx = 0; indx < n_Dof; indx ++ ){
            commands[indx] = 2*(model.nn().get_outf(indx) - 0.5)*p_max; //Is it the right way to obtain new commands
        }
