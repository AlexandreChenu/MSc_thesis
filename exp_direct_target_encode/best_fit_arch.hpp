    ...
    public:
      ...
      void refresh(const E& ea) {
        ...
        if (_cnt%Params::pop::dump_period == 0){ //for each dump period
          ...
          std::cout << "models written 1" << std::endl;}
        ...
        if (_cnt == Params::pop::nb_gen){
          ...
          for (int i=0; i<pop_size; i++){
            ...
            if (i%100 == 0){
              std::cout << "writting model " << std::to_string(i) << std::endl;
            }

            const std::string fmodel = std::string("/git/sferes2/exp/tmp/model_final_") + std::to_string(i) + ".bin";

            {
            std::ofstream ofs(fmodel, std::ios::binary);
            oa_t oa(ofs);
            //oa << model;
            oa << ea.pop()[i]; //check here
            }
          }
          std::cout << "models written 2" << std::endl;
        }
